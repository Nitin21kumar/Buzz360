...
export default function AudioPlayerWidget() {
  ...
  useEffect(() => {
    ...
    let localUrl = null
    setBlobUrl(null)
    if (!folderId || !voices[index]) return undefined
    api.getTtsAudioBlobUrl(folderId, voices[index].filename).then((url) => {
      if (cancelled) {
  ...
  }, [folderId, index, voices])

  const currentFolder = folders.find((f) => f.id === folderId)
  const current = voices[index]

  const goPrev = () => setIndex((i) => (i - 1 + voices.length) % voices.length)
      ...
      {!current ? (
        ...
        <>
          ...
          <div className="audio-waveform">
            ...
            <button onClick={togglePlay} disabled={!blobUrl} className={`audio-play-btn${playing ? ' audio-play-btn--playing' : ''}`}>
