...
import {
  Eye, EyeOff, LockKeyhole, Mail, Mic,
...
import { firebaseAuth } from '../../shared/lib/firebase.js'
...
function AuthView(p) {
  return (
    <main className="auth-page">
      ...
      {p.busy && <PageLoaderOverlay label={p.mode === 'signin' ? 'Signing in' : 'Creating account'} />}
    </main>
  )
}
  ...
  return (
    <form onSubmit={p.submit} className="auth-form">
      ...
      <button disabled={p.busy} className="auth-submit-btn">
        {p.mode === 'signin' ? 'Sign in' : 'Create account'}
      </button>
    </form>
  )
}
