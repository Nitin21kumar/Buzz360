...
> **Migration note:** this project now runs on **PostgreSQL**, not MongoDB
> (see `DATABASE_URL` in `.env.example`), and includes a new **SMS**
> module (ValueFirst/GoInfinito, DLT-template based) alongside TTS/STT/OBD
> calling/WhatsApp. Everything below that still says "MongoDB" is legacy
> text from before that migration — the concepts (audio stored in the
> database, not on disk; streamed back out on demand) are still accurate,
> just read "MongoDB" as "PostgreSQL" and "Mongo document" as "row in
> `app_documents`" (see `backend/app/db/postgres_store.py`).

A fresh, standalone project with four features:
...
```
obd-suite/
├── backend/            FastAPI + PostgreSQL + Sarvam AI (TTS/STT/OBD calling/SMS)
├── whatsapp-service/   Node.js + TypeScript (WhatsApp + RCS broadcast via Sarv APIs)
├── gateway/            nginx reverse proxy — single entrypoint, routes by path
└── frontend/           React + Vite (light theme dashboard)
```
The frontend talks to a single URL (the `gateway` container), which routes
`/api/whatsapp/*` and `/api/rcs/*` to `whatsapp-service` and everything else to `backend`.
This is the first step of a planned gradual migration of the whole backend
from FastAPI to Node.js — see `whatsapp-service/README.md` for details on
why WhatsApp moved first and how the two backends share one Postgres
database safely.
...
5. Click **Apply** — Render builds and deploys both. `SARV_WEBHOOK_SECRET` is auto-generated for you. Once the backend service is live, copy its own URL and set **`PUBLIC_BASE_URL`** on it (e.g. `https://obd-backend-xxxx.onrender.com`, no trailing slash) — this is **required** for OBD calling: without it Sarv gets no status callback and the audio URLs handed to Sarv have no host (calls connect silent). It is not auto-detected.
...
- **Verify calling is fully wired up**: open `https://your-backend.onrender.com/api/campaigns/debug/deployment-check` — shows exactly which piece (`PUBLIC_BASE_URL`, webhook secret, Sarv credentials, Postgres, signing secret) isn't configured yet, and a final `"ready_to_call": true/false`.
